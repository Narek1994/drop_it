...
import com.secrethq.utils.PTGlobalEventCallback;
...
public class PTServicesBridge
	...
	private static native long addGlobalEventListenerNative(int index);
	private static native void removeGlobalEventListenerNative(long callbackReference);
	private static native void clearGlobalEventListenersNative();
	...
	private static List<PTGlobalEventCallback> _callbacks = new ArrayList<>();
	private static Queue<Integer> _removedCallbackIndices = new ArrayDeque<>();
	private static Map<Integer, Long> _callbackReferenceByIndex = new HashMap<Integer, Long>();
	...
	public static void initBridge(Cocos2dxActivity activity, String appId){
		...
		clearGlobalEventListenersNative();
	}
	...
	public static void screenOnEnter(String name){
		...
		CustomIntegrator customAdIntegrator = AdIntegratorManager.getCustomIntegrator();
		CustomIntegrator customAnalyticsIntegrator = AnalyticsIntegratorManager.getCustomIntegrator();
		
		if (customAdIntegrator != null) {
			customAdIntegrator.screenOnEnter(name);
		}

		if (customAnalyticsIntegrator != null) {
			customAnalyticsIntegrator.screenOnEnter(name);
		}
	}
	...
	public static void screenOnExit(String name){
		...
		if (customAdIntegrator != null) {
			customAdIntegrator.screenOnExit(name);
		}

		if (customAnalyticsIntegrator != null) {
			customAnalyticsIntegrator.screenOnExit(name);
		}
	}
	...
	public static void sceneOnEnter(String name){
		...
		if (customAdIntegrator != null) {
			customAdIntegrator.sceneOnEnter(name);
		}

		if (customAnalyticsIntegrator != null) {
			customAnalyticsIntegrator.sceneOnEnter(name);
		}
	}
	...
	public static void sceneOnExit(String name){
		...
		if (customAdIntegrator != null) {
			customAdIntegrator.sceneOnExit(name);
		}

		if (customAnalyticsIntegrator != null) {
			customAnalyticsIntegrator.sceneOnExit(name);
		}
	}
	...
	public static void loadingDidComplete() {
		...
		if (customAdIntegrator != null) {
			customAdIntegrator.loadingDidComplete();
		}

		if (customAnalyticsIntegrator != null) {
			customAnalyticsIntegrator.loadingDidComplete();
		}
	}
	...
	public static void sendHapticFeedback(int level) {
		Log.v(TAG, "PTServicesBridge -- sendHapticFeedback with level " +  level);
		Activity activity = PTServicesBridge.s_activity.get();

        if (activity != null) {
			// On iOS, level 1 = selection feedback and level 2 = impact feedback
			// Since there is no such concept on Android, we will arbitrarily add 150ms per level
			int milliseconds = level * 150;
			Object systemService = activity.getSystemService(VIBRATOR_SERVICE);
    ...
    public static void buttonActivated(String name) {
		CustomIntegrator customAdIntegrator = AdIntegratorManager.getCustomIntegrator();
		CustomIntegrator customAnalyticsIntegrator = AnalyticsIntegratorManager.getCustomIntegrator();
		
		if (customAdIntegrator != null) {
			customAdIntegrator.buttonActivated(name);
		}

		if (customAnalyticsIntegrator != null) {
			customAnalyticsIntegrator.buttonActivated(name);
		}
    }
	
	public static boolean buttonVisible(String name) {
		CustomIntegrator customAdIntegrator = AdIntegratorManager.getCustomIntegrator();
		CustomIntegrator customAnalyticsIntegrator = AnalyticsIntegratorManager.getCustomIntegrator();
		
		if (customAdIntegrator != null && !customAdIntegrator.buttonVisible(name)) {
			return false;
		}

		if (customAnalyticsIntegrator != null && !customAnalyticsIntegrator.buttonVisible(name)) {
			return false;
		}
	...
	public static void addGlobalEventListener(PTGlobalEventCallback callback) {
		Integer removedIndex = _removedCallbackIndices.poll();
		int index;

		if (removedIndex != null) {
			index = removedIndex;
			_callbacks.set(index, callback);
		}
		else {
			index = _callbacks.size();
			_callbacks.add(callback);
		}

		long callbackReference = addGlobalEventListenerNative(index);
		_callbackReferenceByIndex.put(index, callbackReference);
	}

	public static void removeGlobalEventListener(PTGlobalEventCallback callback) {
		int index = _callbacks.indexOf(callback);
		_callbacks.set(index, null);
		_removedCallbackIndices.add(index);

		long callbackReference = _callbackReferenceByIndex.get(index);
		removeGlobalEventListenerNative(callbackReference);
	}

	public static void invokeGlobalEventCallback(int callbackIndex, String eventName, String eventValue) {
		PTGlobalEventCallback callback = _callbacks.get(callbackIndex);

		if (callback != null) {
			callback.run(eventName, eventValue);
		}
	}
}
